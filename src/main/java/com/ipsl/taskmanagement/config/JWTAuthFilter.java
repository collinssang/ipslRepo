...
import com.ipsl.taskmanagement.service.impl.UserServiceImpl;
...
public class JWTAuthFilter extends OncePerRequestFilter{
    ...
    public JWTAuthFilter(UserServiceImpl userDetailsService, ObjectMapper objectMapper) {
...
//       If any accessToken is present, then it will validate the token and then authenticate the request in security context
            ...
            ErrorResponses errorResponse = new ErrorResponses(HttpServletResponse.SC_FORBIDDEN, e.getMessage());
            response.setStatus(HttpServletResponse.SC_FORBIDDEN);
            response.getWriter().write(toJson(errorResponse));
        }
    }

    private String toJson(ErrorResponses response) {
        try {
